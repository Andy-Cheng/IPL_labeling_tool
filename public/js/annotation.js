...
function Annotation(sceneMeta, world, frameInfo){
    ...
    this.preload = function(on_preload_finished){
        ...
        this.load_annotation((frames)=>this.proc_annotation(frames));
        // this.load_prediction((boxes)=>this.proc_prediction(boxes));
    };
    ...
    this._afterPreload = function(type){
        this.preloaded = true;
        console.log(`after preload ${type}`);
    ...
    this.createCuboid = function(pos, scale, rotation, obj_type, track_id, obj_attr, confidence=-1, split_type=""){
        ...
        if (confidence > 0){
            obj_attr = `${split_type}, conf: ${confidence.toFixed(2)}`;
            obj_type += " pred";
    ...
    */
    ...
    this.add_box=function(pos, scale, rotation, obj_type, track_id, obj_attr, confidence=-1){

        let mesh = this.createCuboid(pos, scale, rotation, obj_type, track_id, obj_attr, confidence)
    ...
    this.proc_annotation = function(frame){
        ...
        let split_type = frame.split_type;
        boxes = boxes.filter((box) => !("score" in box) || (box.score>window.pointsGlobalConfig.conf_threshold));
        this.boxes = this.createBoxes(boxes, split_type);  //create in future world
        ...
        this.sort_boxes();
        this._afterPreload("annotation");
    };

    // this.proc_prediction = function(boxes){
    //     // only for debug
    //     let new_boxes = [];
    //     boxes.forEach(box=>{
    //         box['psr']['position']['x'] += 10;
    //         box['psr']['position']['y'] += 10;
    //         new_boxes.push(box)
    //     })
    //     boxes = new_boxes;
    ...
    //     this.webglGroup = new THREE.Group();
    //     this.webglGroup.name = "predictions";
    //     this.boxes.forEach(b=>this.webglGroup.add(b));
    ...
    this.load_annotation=function(on_load){
        if (this.data.cfg.disableLabels){
            ...
            xhr.onreadystatechange = function () {
                ...
                if (this.status == 200) {
                    let frame = _self.frameInfo.anno_to_boxes(this.responseText);
                    on_load(frame);
                }
    ...
    // this.load_prediction=function(on_load){
    //     var xhr = new XMLHttpRequest();
    //     var _self = this;
    //     xhr.onreadystatechange = function () {
    //         if (this.readyState != 4) return;
    ...
    //         if (this.status == 200) {
    //             let pred = _self.frameInfo.anno_to_boxes(this.responseText);
    //             on_load(pred);
    //         }
        
    //         // end of state change: it can be after some time (async)
    //     };
    //     let frame_id = this.frameInfo.frame.split("_")[1];
        
    //     xhr.open('GET', "/load_prediction"+"?scene="+this.frameInfo.scene+"&frame="+frame_id, true);
    //     xhr.send();
    // };
    ...
    this.reapplyAnnotation = function(frame, done){
            ...
            boxes.forEach(nb=>{  // nb is annotation format, not a true box
                ...
                if (old_box){
                    ...
                    // not found
                    let box=this.createOneBoxByAnn(nb, split_type);
    ...
    this.createOneBoxByAnn = function(annotation, split_type=""){
        let b = annotation;
    ...
    this.createBoxes = function(annotations, split_type){
        return annotations.map((b)=>{
            return this.createOneBoxByAnn(b, split_type);
        });
    };

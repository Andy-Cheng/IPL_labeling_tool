...
function FrameInfo(data, sceneMeta, sceneName, frame){
    ...
    this.get_pcd_path = function(){
            let lidar_folder_name = '/os2-64/'

            return 'data/'+ this.scene + lidar_folder_name + this.frame + this.sceneMeta.lidar_ext;
        };
    this.get_radar_path = function(name){
        console.log(name)
        let frame = this.frame.split("_")[1];
        return `data/${this.scene}/radar/${window.pointsGlobalConfig.radar_name}/${frame}${this.sceneMeta.radar_ext}`;
    ...
    this.get_anno_path = function(){
            if (this.annotation_format=="psr"){
                return 'data/'+this.scene + "/"+label_path_name+"/" + this.frame + ".json";
            }
            else{

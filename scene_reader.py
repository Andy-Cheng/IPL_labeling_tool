...
def get_one_scene(s):
    ...
    scene_dir = os.path.join(root_dir, s)
    lidar_folder_name = 'os2-64'
    lidar_path = os.path.join(scene_dir, lidar_folder_name)
    if not os.path.exists(lidar_path):
        lidar_path = os.path.join(scene_dir, 'lidar')
...
def read_annotations(scene, frame):
    ...
    if len(frame_split) > 1:
        frame = frame_split[1]
    filename = os.path.join(root_dir, scene, label_path_name, frame+".json")
